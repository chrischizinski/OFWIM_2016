...
The problem with the default theme in ggplot is that these do not work for presentation formats (e.g., publication in a journal or presentation).  
 ...
 and these are ```theme_bw()``` and ```theme_grey```
...
There is also the package `ggthemes` that offers many different themes and scales for graphics.  Check out the [gallery here](https://cran.r-project.org/web/packages/ggthemes/vignettes/ggthemes.html)
